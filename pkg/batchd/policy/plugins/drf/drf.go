...
func (drf *drfPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	// Register Job Order function.
	ssn.RegisterJobOrderFn(func(l interface{}, r interface{}) int {
		lv := l.(*api.JobInfo)
		rv := r.(*api.JobInfo)
	...
	ssn.AddEventHandler(&framework.EventHandler{
		OnAllocated: func(event *framework.Event) error {
			attr := drf.jobOpts[event.Job.UID]
		...
		OnDeallocated: func(event *framework.Event) error {
			attr := drf.jobOpts[event.Job.UID]
